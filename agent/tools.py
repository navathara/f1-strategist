...
def get_recent_form(driver_id: str) -> dict:
    ...
    try:
        r = requests.get(
            f"{BASE}/current/drivers/{driver_id}/results.json", timeout=10)
        r.raise_for_status()
        ...
        avg_pos = round(sum(positions) / len(positions),
                        1) if positions else 20.0
        return {
            "driver_id": driver_id,
...
def get_circuit_history(circuit_id: str) -> dict:
    ...
    try:
        ...
        for year in [2025, 2024, 2023]:
            r = requests.get(
                f"{BASE}/{year}/circuits/{circuit_id}/results.json?limit=3", timeout=10)
            r.raise_for_status()
            races = r.json()["MRData"]["RaceTable"]["Races"]
            if races:
                race = races[0]
...
def get_value_score(driver_id: str) -> dict:
    ...
    try:
        ...
        driver = next(
            (d for d in standings if d["driver_id"] == driver_id), None)
        if not driver:
            return {"error": f"Driver '{driver_id}' not found. Check driver_id from get_driver_standings."}

        form = get_recent_form(driver_id)
        avg_pos = form.get("avg_position_last_5",
                           10.0) if "error" not in form else 10.0

        leader_points = standings[0]["points"]
        champ_score = (driver["points"] / leader_points *
                       100) if leader_points > 0 else 0
